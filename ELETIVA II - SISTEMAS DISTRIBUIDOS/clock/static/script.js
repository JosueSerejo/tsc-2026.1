let clientCount = 0;
// Stores send times locally so we can display them after sync
const clientSendTimes = {};

async function resetServer() {
    const serverTime = document.getElementById('server-init-time').value;
    ...
    clientCount = 0;
    Object.keys(clientSendTimes).forEach(k => delete clientSendTimes[k]);
    clearResults();
}
...
function addClient() {
    ...
    div.innerHTML = `
        ...
        <input type="text" id="c${clientCount}-local" placeholder="10:30">
        <label>Hora de envio:</label>
        <input type="text" id="c${clientCount}-send" placeholder="10:50">
...
async function sendData(id) {
    ...
    if (res.ok) {
        ...
        document.getElementById(`res-c${id}-send`).innerText = send || '---';
        // Cache send time so synchronize() can look it up
        clientSendTimes[`Cliente ${id}`] = send || '---';
...
async function synchronize() {
    ...
    // Order + send times
    for (let i = 0; i < 3; i++) {
        const clientName = data.order[i] || '?';
        const sendTime   = clientName !== '?' ? (clientSendTimes[clientName] || '---') : '---';
        document.getElementById(`order-${i + 1}`).innerText      = clientName;
        document.getElementById(`order-${i + 1}-send`).innerText = sendTime;
    }
}

function clearResults() {
    ['res-c1-time','res-c2-time','res-c3-time',
     'res-c1-send','res-c2-send','res-c3-send',
     'logical-clock','adj-s','adj-c1','adj-c2','adj-c3'].forEach(id => {
        document.getElementById(id).innerText = '---';
    });
    ['order-1','order-2','order-3'].forEach(id => {
        document.getElementById(id).innerText = '?';
        document.getElementById(`${id}-send`).innerText = '---';
    });
}